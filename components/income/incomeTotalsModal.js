...
const IncomeTotalsModal = ({ incomeTotals, totalsModal, setTotalsModal }) => {
  const closeIncomeModal = () => {
    setTotalsModal(false);
  };

  return (
    <Modal show={totalsModal} onHide={closeIncomeModal} centered>
      ...
      <Modal.Body>
        <Table striped>
          ...
          <tbody>
            {incomeTotals.types.map((type, index) => (
              <tr key={index} className="d-flex">
                <td className="col-6 col-md-3 fw-bold">{type.type}</td>
...
export default IncomeTotalsModal;
